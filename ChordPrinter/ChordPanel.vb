Public Class ChordPanel
    ...
    Public Overloads ReadOnly Property Focused As Boolean
        Get
            If FocusChord = Key Then
                Return True
            Else
                Return False
            End If
    ...
    Private Sub OnKey(ByVal sender As Object, ByVal e As PreviewKeyDownEventArgs) Handles MyBase.PreviewKeyDown
        Select e.KeyCode
            Case Keys.Enter
                If FocusChord <> "" Then
                    EditChord(Chords(FocusChord))
                    bChanged = True
                End If
            Case Keys.Left
                For i As Integer = 2 To Chords.Count
                    If Chords(i).Key = FocusChord Then
                        FocusChord = Chords(i - 1).Key
                        frmChordPrinter.BodyArea.Invalidate(True)
                        frmChordPrinter.RhythmArea.Invalidate(True)
                        Exit For
                    End If
